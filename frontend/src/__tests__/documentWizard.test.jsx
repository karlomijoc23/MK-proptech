...
test("shows inline error when manual unit is submitted without property", async () => {
  ...
  fireEvent.click(
    within(manualForm).getByRole("button", { name: /Spremi podprostor/i }),
  );

  expect(
    screen.getByText(/Odaberite nekretninu prije spremanja podprostora/i),
  ).toBeInTheDocument();
...
test("creates manual unit when property is selected", async () => {
  ...
  expect(toast.success).toHaveBeenCalledWith("Podprostor je kreiran.");
});
