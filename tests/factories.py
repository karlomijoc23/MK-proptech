...
def create_invoice(client, headers, **overrides) -> Dict[str, Any]:
    today = date.today()
    payload = {
        "nekretnina_id": overrides.get("nekretnina_id"),
        "ugovor_id": overrides.get("ugovor_id"),
        "property_unit_id": overrides.get("property_unit_id"),
        "tip_rezije": overrides.get("tip_rezije", "struja"),
        "dobavljac": overrides.get("dobavljac", "HEP"),
        "broj_racuna": overrides.get("broj_racuna", "INV-001"),
        "razdoblje_od": overrides.get("razdoblje_od", today.replace(day=1).isoformat()),
        "razdoblje_do": overrides.get("razdoblje_do", today.isoformat()),
        "datum_izdavanja": overrides.get("datum_izdavanja", today.isoformat()),
