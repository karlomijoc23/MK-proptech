...
2. **Kreiranje baze i korisnika:**
   ...
   ```sql
   CREATE DATABASE mkproptech CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
   CREATE USER 'mkproptech'@'localhost' IDENTIFIED BY 'vasa_lozinka';
   GRANT ALL PRIVILEGES ON mkproptech.* TO 'mkproptech'@'localhost';
   FLUSH PRIVILEGES;
   EXIT;
   ```
...
Pretpostavljamo da ste klonirali kod u `/opt/mk-proptech` ili svoj home direktorij.
...
3. **Konfiguracija okoline (.env):**
   ...
   ```env
   # Baza podataka
   DATABASE_URL=mariadb+asyncmy://mkproptech:vasa_lozinka@localhost/mkproptech
