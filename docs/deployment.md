...
This document walks through deploying the MK-proptech stack using Render for the FastAPI backend (and optional static hosting) plus Vercel for the React frontend.
...
1. **Create secrets (Render Dashboard → Secrets):**
   ...
   - Integrations (`mk-proptech-openai`, `mk-proptech-mailgun`) as needed.
...
5. **CORS:** update `CORS_ORIGINS` env var (in `render.yaml` or dashboard) to your public frontend URL.
...
- Update `CORS_ORIGINS` and Vercel env facts accordingly.
